...
<body>
	<div class="pagingArea" align="center">
		...
		<c:forEach var="p" begin="${ requestScope.noticePaging.startPage }" end="${ requestScope.noticePaging.endPage }" step="1">
			...
			<c:if test="${ requestScope.noticePaging.currentPage ne p }">
				<button onclick="location.href='${ pageContext.servletContext.contextPath }/application/select/fail?page=<c:out value="${ p }"/>&&criteria=<c:out value="${ requestScope.criteria }"/>'"><c:out value="${ p }"/></button>
			</c:if>
		</c:forEach>
	...
	<script>
		...
		const link = "${ pageContext.servletContext.contextPath }/application/select/fail";
