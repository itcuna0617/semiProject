...
<body>
	...
	<script>
		...
		const link = "${ pageContext.servletContext.contextPath }/postingList";
		let searchText = "";
		
		if(${ !empty requestScope.selectCriteria.searchCondition1? true: false }) {
			searchText += "&searchCondition=${ requestScope.selectCriteria.searchCondition1 }";
		}
		
		if(${ !empty requestScope.selectCriteria.searchCondition2? true: false }) {
			searchText += "&searchCondition=${ requestScope.selectCriteria.searchCondition2 }";
		}
		...
		if(document.getElementById("maxPage")) {
			...
			$maxPage.onclick = function() {
				...
				// 보고자 하는 페이지에 검색어가 채워져있으면 쿼리스트링 형태로 달고 가는 것 (주소창에 내역이 누적)
				// 검색어 유지
			}
		}
