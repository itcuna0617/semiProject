...
@WebServlet("/application/select/fail")
public class SelectApplicationFailByEnt extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int total=appService.selectFailTotalCount(criteriaMap);
		NoticePagenation pagenation = new NoticePagenation(total, size, currentPage, pagingCount);
		
		List<ApplicationDTO> applicationList=appService.selectApplicationFail(criteriaMap);
		...
		path="/WEB-INF/views/posting/selectApplicationFailByEnt.jsp";
		if("posting".equals(criteria)) {
