...
@WebServlet("/postingList")
public class PostingListServlet extends HttpServlet {
...
/* 현재 페이지 불러오기 */
		...
		int totalCount = postingService.selectTotalCount(searchMap);
		
		System.out.println("totalPostingCount : " + totalCount);
...
//		System.out.println(searchCondition1 != null && searchCondition2 != null && !"".equals(searchValue));
		...
		List<PostingDTO> postingList = postingService.selectPostingList(selectCriteria);
		...
		String path = "";
		if(postingList != null) {
			path = "/WEB-INF/views/posting/postingList.jsp";
